...
def display_test_results(input_img, density_map, output_dir, fname='results.png', estimation=0):
    input_img = input_img[0][0]
    density_map = 255 * density_map / np.max(density_map)
    density_map = density_map[0][0]
    if density_map.shape[1] != input_img.shape[1]:
    ...
    result_img = result_img.astype(np.uint8, copy=False)
    if estimation != 0:
        detection_text = 'Total Detections : ' + str(estimation)
        cv2.putText(result_img, detection_text, (10, 50), font, 0.5, (255, 255, 255), 1, cv2.LINE_AA)
        cv2.imwrite(os.path.join(output_dir, fname), result_img)
    # cv2.imshow('Result', result_img)
    # cv2.waitKey(0)
