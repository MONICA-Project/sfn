...
class ImageDataLoader():
    def __init__(self, data_path, gt_path, shuffle=False, gt_downsample=False, pre_load=False, num_classes=10):
        ...
        # REMOVE SOME FILES FROM THE data_files LIST IF THEY ARE .mat OR .CSV FILES
        mask = [('csv' not in file) and ('mat' not in file) for file in self.data_files]
        self.data_files = [self.data_files[i] for i in range(len(self.data_files)) if mask[i]]
        ...
        if self.pre_load:
            # load input images and ground truth into memory
            self.preload_data()
            # assign ground truth crowd group/class labels to each image
            self.assign_gt_class_labels()
